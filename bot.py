...
#dari baris 1 sampai 4 untuk mengimpor
...
@bot.event
async def on_ready():
    print(f'Bot is ready. Logged in as {bot.user}')# ini dipanggil kalau bot berhasil online

@bot.command(name='start')
async def start_command(ctx):
    await ctx.send("Halo! Saya adalah bot manajer proyek\nSaya akan membantu kamu menyimpan proyek dan informasi tentangnya!)") # Kirim pesan perkenalan bot 
    await info(ctx)# tampilkan info

@bot.command(name='info')
async def info(ctx):# Command menampilkan daftar perintah bot
    await ctx.send("""
...
async def new_project(ctx):# Command membuat proyek baru
    await ctx.send("Masukkan nama proyek:")# meminta nama proyek

    def check(msg):
        return msg.author == ctx.author and msg.channel == ctx.channel

    name = await bot.wait_for('message', check=check)
    data = [ctx.author.id, name.content]# Simpan user_id dan nama proyek
    await ctx.send("Masukkan link proyek")# meminta link proyek
    link = await bot.wait_for('message', check=check)
    data.append(link.content)

    statuses = [x[0] for x in manager.get_statuses()]
    await ctx.send("Masukkan status proyek saat ini", delete_after=60.0)#meminta status dalam proyek
    await ctx.send("\n".join(statuses), delete_after=60.0) # kirim daftar status yang bisa dipilih 
    
    status = await bot.wait_for('message', check=check)
    if status.content not in statuses:## pastikan input ada di daftar status
...
async def get_projects(ctx):#berfungsi menampilkan daftar proyek
...
async def skills(ctx):#berfungsi menambahkan skills
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:
...
async def delete_project(ctx):#berfungsi menghapus proyek
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]
...
async def update_projects(ctx):#berfungsi mengubah data proyek
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]
